...
- Define `Props` interface in frontmatter — every reusable component **must** document this interface so its API is self-explanatory. Add a JSDoc comment above the interface and a comment on any prop whose purpose is not obvious from its name and type. See [`coding-standards.instructions.md`](coding-standards.instructions.md) for examples.
